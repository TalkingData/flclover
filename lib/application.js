...
module.exports = class Application extends KoaApplication {
  ...
  loadMiddleware() {
    ...
    if (middlewares) {
      middlewares.forEach((middleware) => {
        this.use(utils.loadFile(`${this.baseDir}/app/middleware/${middleware}`)(this.config[middleware]));
      });
    }
    ...
    this.use(FlcloverProxy(this.config.flcloverProxy));
  ...
  loadController() {
    ...
    fs.readdirSync(`${this.baseDir}/app/controller`).forEach((file) => {
      controller[utils.camelize(file.slice(0, -3))] = utils.loadFile(`${this.baseDir}/app/controller/${file}`);
    });
    utils.loadFile(`${this.baseDir}/app/router`)(this.router, controller);
